...
rule all:
    input:
        ...
        expand("{output_dir}/annotation_feat/abricate/{sample}/{sample}_abricate.txt", output_dir=output_dir, sample=samples) if config["params"].get("abricate_db_dir") else [], 
        f"{eggnog_db}",
...
rule dorado_basecalling:
    ...
    params:
        ...
        processing_unit=dorado_pu
    conda:
        "envs/basecalling.yaml"
    priority: 45
    shell:
        """
        dorado basecaller {params.models[0]} {input} --modified-bases-models {params.models[1]},{params.models[2]} -x {params.processing_unit} > {output}
        """
